...
// Environmental Physiology calculators array
const calculators = [
  {
    name: "Thermal Stress Index Calculator",
    route: "/environmental-physiology/calculator/thermal-stress",
  },
  {
    name: "Acclimatization Assessment Tool",
    route: "/environmental-physiology/calculator/acclimatization",
  },
  {
    name: "Altitude Adjustment Calculator",
    route: "/environmental-physiology/calculator/altitude-adjustment",
  },
  {
    name: "Hydration Needs Calculator",
    route: "/environmental-physiology/calculator/hydration-needs",
  },
  {
    name: "Environmental Load Estimator",
    route: "/environmental-physiology/calculator/environmental-load",
  },
];

export default function EnvironmentalPhysiologyPage(): JSX.Element {
  ...
  // Fetch Environmental Physiology data using the topic service
  const fetchFreshData = useCallback(async () => {
    try {
      const subTopics = await topicService.GetSubTopics("environmental-physiology");
      const latest = await topicService.GetLatestPosts("environmental-physiology", 5);
  ...
  const handleFileClick = useCallback((postName: string): void => {
    router.prefetch(`/environmental-physiology/posts/${postName}`);
    router.push(`/environmental-physiology/posts/${postName}`);
  }, [router]);
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <Image
          src="/images/environmental-physiology.webp"
          alt="Environmental Physiology Background"
          fill
          priority
      ...
      <header ref={breadcrumbRef} className="mx-4 sm:mx-6 lg:mx-16 mt-6">
        <BreadcrumbNavigation
          crumbs={[
            { label: "Home", href: "/" },
            { label: "Environmental Physiology", href: "/environmental-physiology" },
          ]}
        />
      </header>
      ...
      <main className="relative z-10 mx-4 sm:mx-6 lg:mx-16 my-6 p-4 sm:p-6 lg:p-8 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm rounded-[20px] sm:rounded-[30px] shadow-lg">
        ...
        <section ref={heroContentRef} className="flex flex-col items-center text-center py-4 sm:py-6 mb-6">
          ...
          <p className="mt-2 text-sm sm:text-base md:text-lg text-gray-700 dark:text-gray-300 max-w-xl">
            Explore how environmental factors impact human physiology and performance.
          </p>
        </section>
