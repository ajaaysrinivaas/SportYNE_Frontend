...
// Strength and Conditioning calculators array
const calculators = [
  { name: "1RM Calculator", route: "/strength-conditioning/calculator/one-rep-max" },
  { name: "Volume Load Calculator", route: "/strength-conditioning/calculator/volume-load" },
  { name: "Progressive Overload Planner", route: "/strength-conditioning/calculator/progressive-overload" },
  { name: "Energy Expenditure Estimator", route: "/strength-conditioning/calculator/energy-expenditure" },
  { name: "Training Density Calculator", route: "/strength-conditioning/calculator/training-density" },
];

export default function StrengthConditioningPage(): JSX.Element {
  ...
  // Fetch data using TopicService for the "strength-conditioning" topic
  const fetchFreshData = useCallback(async () => {
    try {
      const subTopics = await topicService.GetSubTopics("strength-conditioning");
      const latest = await topicService.GetLatestPosts("strength-conditioning", 5);
  ...
  const handleFileClick = useCallback((fileId: string): void => {
    router.prefetch(`/strength-conditioning/file/${fileId}`);
    router.push(`/strength-conditioning/file/${fileId}`);
  }, [router]);
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <Image
          src="/images/strength-conditioning.webp"
          alt="Strength and Conditioning Background"
          fill
          priority
      ...
      <header ref={breadcrumbRef} className="mx-4 sm:mx-6 lg:mx-16 mt-6">
        <BreadcrumbNavigation
          crumbs={[
            { label: "Home", href: "/" },
            { label: "Strength and Conditioning", href: "/strength-conditioning" },
          ]}
        />
      </header>
      ...
      <main className="relative z-10 mx-4 sm:mx-6 lg:mx-16 my-6 p-4 sm:p-6 lg:p-8 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm rounded-[20px] sm:rounded-[30px] shadow-lg">
        ...
        <section ref={heroContentRef} className="flex flex-col items-center text-center py-4 sm:py-6 mb-6">
          ...
          <p className="mt-2 text-sm sm:text-base md:text-lg text-gray-700 dark:text-gray-300 max-w-xl">
            Optimize your training with tools for strength, endurance, and performance.
          </p>
        </section>
