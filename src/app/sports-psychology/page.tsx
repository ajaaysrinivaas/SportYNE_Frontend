...
// Sports Psychology questionnaires array
const questionnaires = [
  { name: "Mental Toughness Questionnaire", route: "/sports-psychology/questionnaire/mental-toughness" },
  { name: "Athlete Stress Assessment", route: "/sports-psychology/questionnaire/stress-assessment" },
  { name: "Motivation Scale", route: "/sports-psychology/questionnaire/motivation-scale" },
  { name: "Focus and Concentration Inventory", route: "/sports-psychology/questionnaire/focus-concentration" },
  { name: "Team Cohesion Survey", route: "/sports-psychology/questionnaire/team-cohesion" },
];

export default function SportsPsychologyPage(): JSX.Element {
  ...
  // Fetch data using TopicService for the "sports-psychology" topic
  const fetchFreshData = useCallback(async () => {
    try {
      const subTopics = await topicService.GetSubTopics("sports-psychology");
      const latest = await topicService.GetLatestPosts("sports-psychology", 5);
  ...
  const handleFileClick = useCallback((fileId: string): void => {
    router.prefetch(`/sports-psychology/file/${fileId}`);
    router.push(`/sports-psychology/file/${fileId}`);
  }, [router]);
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <Image
          src="/images/sports-psychology.webp"
          alt="Sports Psychology Background"
          fill
          priority
      ...
      <main className="relative z-10 mx-4 sm:mx-6 lg:mx-16 my-6 p-4 sm:p-6 lg:p-8 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm rounded-[20px] sm:rounded-[30px] shadow-lg">
        ...
        <section ref={heroContentRef} className="flex flex-col items-center text-center py-4 sm:py-6 mb-6">
          ...
          <p className="mt-2 text-sm sm:text-base md:text-lg text-gray-700 dark:text-gray-300 max-w-xl">
            Enhance mental performance and resilience through psychological insights.
          </p>
        </section>

        {/* Content Grid */}
        <section ref={contentSectionRef} className="grid grid-cols-1 lg:grid-cols-[75%_25%] gap-4 sm:gap-6">
          {/* Left Column: Subtopics & Questionnaires */}
          <div className="flex flex-col gap-6">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-700 dark:text-gray-300">
                Utilize our sports psychology questionnaires for personalized insights.
              </p>
              <ToolsList title="" description="" tools={questionnaires} />
            </div>
          </div>
